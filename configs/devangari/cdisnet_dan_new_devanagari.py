...
from vedastr.attention_masks.masks import generate_square_subsequent_mask, src_mask_attend_only_neighbour_tokens, \
	generate_token_mask, generate_sem_token_mask, generate_token_mask2
...
batch_max_length = 70
test_folder_names = [ "kaggle_train"]  ###
...
validation_folder_names = ["kaggle_train"]

mj_folder_names = ['MJ_test', 'MJ_train']

real_world_train_folders = ["1", "2", "3", "4", "5", "6", "7", "kaggle_val",
                            "icdar_hindi", "IIIT"]
	...
	model=dict(
			...
			pipelines=[
					...
					arch=dict(
						encoder=dict(
							backbone=dict(
								...
								maxT=int(batch_max_length / 7)*4 + 1,               # todo move this number of 7 to
								# some variable
								depth=8,
								num_channels=64
							),  # c1
				...
				dict(
					...
					arch=dict(
						...
						max_len=int(batch_max_length / 7)*4 + 1,
						dropout=dropout
					)
				)
		...
		mdcdp_layers=[
			dict(
				...
				sae_config=dict(
					...
					encoder_layer=dict(
						...
						attention=dict(
							...
							in_channels=hidden_dim,
							k_channels=hidden_dim // n_head_vis,
							v_channels=hidden_dim // n_head_vis,
							n_head=n_head_vis,
							dropout=dropout,
						),
				...
				cbi_s_config=dict(
					...
					encoder_layer=dict(
						...
						attention=dict(
							...
							k_channels=hidden_dim_cbi // n_head_sem,
							v_channels=hidden_dim_cbi // n_head_sem,
							n_head=n_head_sem,
							dropout=dropout,
						),
				...
				cbi_v_config=dict(
					...
					encoder_layer=dict(
						...
						attention=dict(
							type='MultiHeadAttention',
							in_channels=hidden_dim_cbi,
							k_channels=hidden_dim_cbi // n_head_vis,
							v_channels=hidden_dim_cbi // n_head_vis,
							n_head=n_head_vis,
							dropout=dropout,
						),
						attention_norm=layer_norm_cbi,
				...
				vis_mask=generate_token_mask2,
				sem_mask=generate_sem_token_mask,
				vis_mask_range=(7,8),                      #for 7 consecutinve source token attend to 2 extra token 1
...
train = dict(
	...
	resume=dict(checkpoint="/home/shubham/Documents/MTP/text-recognition-models/vedastr/tools/workdir"
	                       "/cdisnet_dan_new_devanagari/best_acc.pth")
